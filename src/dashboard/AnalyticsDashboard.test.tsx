...
describe('AnalyticsDashboard — User Journey', () => {
  ...
  it('renders User Journey and issues a visitorId fetch when a visitor is selected', async () => {
    const mockFetch = vi.fn().mockImplementation(async (url: string) => {
      if (String(url).includes('visitorId')) {
    ...
    const journeyCall = mockFetch.mock.calls.find(([url]: [string]) => String(url).includes('visitorId'))
    expect(journeyCall).toBeTruthy()
    const calledUrl = String(journeyCall[0])
    expect(calledUrl).toContain('visitorId=aaaabbbb-1111-2222-3333-ccccddddeeee')
    expect(calledUrl).not.toContain('funnelSteps')
  })
})
